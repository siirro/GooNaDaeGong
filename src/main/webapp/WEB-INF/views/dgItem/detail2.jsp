...
<body>
	...
	<form role="form" action="./detail" method="post">
		...
		<input type="hidden" name="item_num"
			value="${dgItemDetailDTO.item_num}"> <label>1그룹</label> <span
			class="cate1"></span> <label>2그룹</label> <span class="cate2"></span>
		<label>3그룹</label> <span class="cate3"></span> <label>4그룹</label> 
		<span class="cate4">${dgItemDetailDTO.cate_num}</span>
		...
		<div class="inputArea">
			<label for="item_price">상품가격</label> <span> <fmt:formatNumber
					value="${dgItemDetailDTO.item_price}" pattern="###,###,###" />
			</span>
		</div>
		...
		<div id="reply">
			...
			<c:forEach items="${rvDTO}" var="rvDTO">
				<table>
					<tr>
						<td>${rvDTO.rv_num}</td>
						<td>${rvDTO.rv_title}</td>
						<td>${rvDTO.user_id}</td>
						<td>${rvDTO.rv_date}</td>
					</tr>
					</c:forEach>
				...
				<button type="button">
					<a href="./review?item_num=${dgItemDetailDTO.item_num}">후기</a>
				</button>
		</div>
	...
	<!-- <script>
		...
		$("#update_btn").click(function() {
			formObj.attr("action", "./update?item_num=${dgItemDTO.item_num }");
